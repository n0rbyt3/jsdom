...
module.exports = class ResourceLoader {
  ...
  _getRequestOptions(urlString, scheme, { cookieJar, referrer, accept = "*/*" }) {
    ...
    if (referrer && !IS_BROWSER) {
      // FIXME: "referer" is expected by test/api/from-url, should be "Referer" instead
      ...
      return fetch(uri, requestOptions)
        .then(response => {
          ...
          if (response.status >= 300 && response.status < 400) {
            ...
            // FIXME: "referer" is expected by tests, should be "Referer"
            requestOptions.headers["referer"] = uri;
